...
let make_prologue consts_tbl fvars_tbl =
  ...
  [
    ...
    "apply", "apply"; "car", "car"; "cdr", "cdr"; "set-car!", "set_car"; "set-cdr!", "set_cdr"; "cons", "cons";
  ] in
  ...
  let constant_bytes (c, (a, s)) =
    ...
    s in
  (* Filter out all primitive lables from primitive_names_to_labels that don't appear in the free variables table,
     there's no need to make a primitive closure for them (actually, it's an error) *)
  let filtered_primitive_names_to_labels = List.filter (function (prim, _) -> List.mem_assoc prim fvars_tbl) primitive_names_to_labels in
  let primitive_closures = List.map make_primitive_closure filtered_primitive_names_to_labels in
